...
class ConversationItem extends StatefulWidget {
  ...
  ConversationItem(Conversation conversation, ConversationHandler handler) :
        conversation = conversation, handler = handler,
...
class ConversationItemState extends State<ConversationItem> {
      ...
      leading: new Container(
          ...
          decoration: new BoxDecoration(
              ...
              image: new DecorationImage(
                  fit: BoxFit.fill,
                  image: _getAvatarWidget()
              )
          )),
      title: new Row(
        children: <Widget>[
          new Expanded(child: new Text(_getTitle(), overflow: TextOverflow.ellipsis, maxLines: 1,)),
        ],
      ),
  ...
  String _getDate() {
    DateTime messageDate = DateTime.fromMillisecondsSinceEpoch(conversation.lastMessage.date * 1000);
  ...
  Widget _getLastMessage() {
    if(conversation.conversationInfo.peer.isUser()
      ...
      if(conversation.conversationInfo.peer.localId == conversation.lastMessage.fromId) {
        return new Text(
            conversation.lastMessage.text, overflow: TextOverflow.ellipsis,
            maxLines: 1);
      } else {
            ...
            children: <Widget>[
              new Container(
                  ...
                  decoration: new BoxDecoration(
                      ...
                      image: new DecorationImage(
                          fit: BoxFit.fill,
                          image: new NetworkImage(handler.profiles[conversation.lastMessage.fromId].avatar)
                      )
                  )),
    new Expanded(child: new Text(conversation.lastMessage.text, overflow: TextOverflow.ellipsis, maxLines: 1))
            ],
        );
      }
    } else if(conversation.conversationInfo.peer.isChat()){
